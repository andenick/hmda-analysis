"""
Bhutta (2011) RD Replication - Version 3 (Correct Census Data)

Key improvements over v2:
1. Uses correctly parsed FFIEC census data with valid OOU values
2. Aggregates to TRACT-LEVEL (sum 1994-2002), matching Bhutta exactly
3. Applies exact Bhutta filters from Section 4.1 of the paper
4. Uses 1996 census as base (1990 Census characteristics)
...
CENSUS_DIR = Path(
    str(OUTPUT_ROOT / "Technical/data/census_parsed")
)  # NEW: Correctly parsed data
OUTPUT_DIR = Path(
    str(DATA_ROOT / "Technical/src/bhutta_replication/r_modified/output_v3")
)
...
def load_census_1996() -> pd.DataFrame:
    """
    Load base census characteristics from correctly parsed FFIEC data.
    ...
    print(f"  Mean Total HU: {df['total_housing_units'].mean():.1f}")
...
def apply_bhutta_filters(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 4. Require some lending activity
    result = result[result["num_loans"] > 0]
    print(f"    After loans > 0: {len(result):,} ({len(result)/n_start*100:.1f}%)")

    # 5. Originations per OOU filter: 0.25 to 20 (over entire 1994-2002 period)
...
def create_analysis_vars(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Control variables (matching Bhutta Table 1)
...
def main():
    """Run improved Bhutta replication analysis with correct census data."""
    print("=" * 70)
    print("BHUTTA (2011) RD REPLICATION - VERSION 3 (CORRECT CENSUS DATA)")
    print("=" * 70)
    print()
    ...
    # ===== STEP 3: Load Census =====
    print("\nStep 3: Loading census data (correctly parsed FFIEC 1996)...")
    ...
    if "error" not in res:
        ...
        print(f"    SE: {res['se']:.4f} (target: {target['se']:.4f})")
        print(f"    N: {res['n']:,} (target: {target['n']:,})")
        print(f"    R²: {res['r2']:.3f} (target: ~0.869)")
        coef_diff = res["coef"] - target["coef"]
    ...
    results_df.to_csv(OUTPUT_DIR / "regression_comparison_v3.csv", index=False)
    print(f"\nResults saved to: {OUTPUT_DIR / 'regression_comparison_v3.csv'}")
